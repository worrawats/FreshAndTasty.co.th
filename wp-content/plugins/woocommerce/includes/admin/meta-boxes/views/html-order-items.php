...
//{
	//$arraycoupon = $order->get_used_coupons();
	//$coupon_code = 'agent_order';
	//$kupon = new WC_Coupon($coupon_code );
	//$amount = $kupon->coupon_amount;
	//if (!in_array("agent_order", $arraycoupon)) {
	//	$order->add_coupon($coupon_code, $amount);
	//}
...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
	...
	$post = get_post($order);
	$postd = $order->post;
	//var_dump($postd->post_author);
	//$post->
	$user = new WP_User($postd->post_author);
		...
		<div class="wc-used-coupons">
			<ul class="wc_coupon_list"><?php
				...
				foreach ( $coupons as $item_id => $item ) {
					...
					if($user->roles[0]!="shop_manager")
					{
						echo '<a href="' . esc_url( $link ) . '" class="tips" data-tip="' . esc_attr( wc_price( $item['discount_amount'], array( 'currency' => $order->get_order_currency() ) ) ).'">';
					}
					echo '<span>' . esc_html( $item['name'] ). '</span></a></li>';
				}
				?></ul>
		</div>
		<?php
	...
	if($user->roles[0]=="shop_manager") {
		$discount = round((($total_all * 30) / 100), 2);
		$fordistributor = " for Distributor 30%";
	}
	else{
	...
	<table class="wc-order-totals">
		...
		<tr>
			<td class="label"><?php echo wc_help_tip( __( 'ราคารวม', 'woocommerce' ) ); ?> <?php _e( 'Total Before Discount', 'woocommerce' ); ?>:</td>
			<td width="1%"></td>
			<td class="total">
				<?php echo wc_price($total_all, array( 'currency' => $order->get_order_currency() ) ); ?>
			</td>
		</tr>
		<tr>
			<td class="label"><?php echo wc_help_tip( __( 'ราคาลด', 'woocommerce' ) ); ?> <?php _e( 'Discount'.$fordistributor, 'woocommerce' ); ?>:</td>
			<td width="1%"></td>
			<td class="total">
				<?php echo '-'.wc_price( $discount, array( 'currency' => $order->get_order_currency() ) ); ?>
			</td>
		</tr>
		...
		<tr>
			...
			<td class="total">
				<div class="view"><?php
					//var_dump($data['_order_total'][0]);
					$total_all2 =  $total_all;
					if($user->roles[0]=="shop_manager") {
						$totalnow = $total_all2 - $discount;
					}
					else{
						$totalnow = $order->get_total();
					}
					echo wc_price($totalnow, array( 'currency' => $order->get_order_currency() ) ); ?></div>
				<div class="edit" style="display: none;">
					<input type="text" class="wc_input_price" id="_order_total" name="_order_total" placeholder="<?php echo wc_format_localized_price( 0 ); ?>"
						   value="<?php
						   if($user->roles[0]=="shop_manager") {
							   echo $totalnow;
						   }else{
