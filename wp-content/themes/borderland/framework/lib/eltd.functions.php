 ...
 * It first checks if option exists in $eltd_options global array and if it does'nt exists there
 * it checks default theme options array.
 *
 * @param $name string name of the option to retrieve
 * @return mixed|void
 */
function eltdf_option_get_value($name) {
	global $eltd_options;
	global $eltdFramework;

	if (array_key_exists($name, $eltdFramework->eltdOptions->options)) {
		if(isset($eltd_options[$name])){
			return $eltd_options[$name];
		} else {
 ...
 */
function eltdf_generate_filename( $file, $w, $h ){
	$info         = pathinfo( $file );
	$dir = "";
	...
	if (eltdf_url_exists("{$dir}/{$name}-{$suffix}.{$ext}"))
 ...
 * @return bool|string
 *
 * @see eltd_get_attachment_id_from_url()
 */
function eltdf_get_attachment_thumb_url($attachment_url) {
	$attachment_id = eltd_get_attachment_id_from_url($attachment_url);
 ...
 */
function eltd_enqueue_skin_style($handle, $src, $deps = array(), $ver = false, $media = 'all') {
	global $eltdFramework;
 ...
 */
function eltd_register_skin_style($handle, $src, $deps = array(), $ver = false, $media = 'all') {
	global $eltdFramework;
 ...
 */
function eltd_enqueue_skin_script($handle, $src, $deps = array(), $ver = false, $in_footer = false) {
	global $eltdFramework;
 ...
 */
function eltd_register_skin_script($handle, $src, $deps = array(), $ver = false, $in_footer = false) {
	global $eltdFramework;
...
if (!function_exists('eltd_generate_dynamic_css_and_js')){
	/**
	 * Function that gets content of dynamic assets files and puts that in static ones
	 */
	function eltd_generate_dynamic_css_and_js() {

		$eltd_options = get_option('eltd_options_borderland');
		if(eltd_is_css_folder_writable()) {
	...
	if(!is_multisite()) {
		add_action('eltd_after_theme_option_save', 'eltd_generate_dynamic_css_and_js');
	}
}

if (!function_exists('eltd_gallery_upload_get_images')) {
	/**
	 ...
	 */
	function eltd_gallery_upload_get_images(){
		$ids=$_POST['ids'];
		$ids=explode(",",$ids);
		foreach($ids as $id):
	...
	add_action( 'wp_ajax_eltd_gallery_upload_get_images', 'eltd_gallery_upload_get_images');
}

if (!function_exists('eltd_hex2rgb')) {
	/**
...
if(!function_exists('eltd_get_attachment_meta')) {
	/**
	 ...
	 * @return array|mixed
	 */
	function eltd_get_attachment_meta($attachment_id, $keys = array()) {
		$meta_data = array();
...
if(!function_exists('eltd_get_attachment_id_from_url')) {
	/**
	 * Function that retrieves attachment id for passed attachment url
	 * @param $attachment_url
	 * @return null|string
	 */
	function eltd_get_attachment_id_from_url($attachment_url) {
		global $wpdb;
		$attachment_id = '';
...
if(!function_exists('eltd_get_attachment_meta_from_url')) {
	/**
	 ...
	 *
	 * @see eltd_get_attachment_id_from_url()
	 * @see eltd_get_attachment_meta()
	 *
	 * @version 0.1
	 */
	function eltd_get_attachment_meta_from_url($attachment_url, $keys = array()) {
		...
		if(!empty($attachment_id)) {
			//get post meta
			$attachment_meta = eltd_get_attachment_meta($attachment_id, $keys);
		}
...
if(!function_exists('eltd_get_image_dimensions')) {
	/**
	 ...
	 *
	 * @see eltd_get_attachment_meta_from_url()
	 * @uses getimagesize
	 *
	 * @version 0.1
	 */
	function eltd_get_image_dimensions($url) {
		$image_sizes = array();

		//is url passed?
		if($url !== '') {
			//get image sizes from posts meta if attachment exists
			$image_sizes = eltd_get_attachment_meta_from_url($url, array('width', 'height'));
...
if(!function_exists('eltd_get_native_fonts_list')) {
	/**
	 * Function that returns array of native fonts
	 * @return array
	 */
	function eltd_get_native_fonts_list(){
...
if(!function_exists('eltd_get_native_fonts_array')) {
	/**
	 * Function that returns formatted array of native fonts
	 *
	 * @uses eltd_get_native_fonts_list()
	 * @return array
	 */
	function eltd_get_native_fonts_array(){
		$native_fonts_list = eltd_get_native_fonts_list();
		$native_font_index = 0;
...
if(!function_exists('eltd_is_native_font')) {
	/**
	 * Function that checks if given font is native font
	 * @param $font_family string
	 * @return bool
	 */
	function eltd_is_native_font($font_family) {
		return  in_array(str_replace('+', ' ', $font_family), eltd_get_native_fonts_list());
	}
}


if(!function_exists('eltd_merge_fonts')) {
	/**
	 * Function that merge google and native fonts
	 *
	 * @uses eltd_get_native_fonts_array()
	 * @return array
	 */
	function eltd_merge_fonts() {
		global $eltd_fonts_array;
		$native_fonts = eltd_get_native_fonts_array();

		if(is_array($native_fonts) && count($native_fonts)){
			$eltd_fonts_array = array_merge($native_fonts, $eltd_fonts_array);
		}
	}

	add_action('admin_init', 'eltd_merge_fonts');
}

if(!function_exists('eltd_is_css_folder_writable')) {
	/**
	 ...
	 * @uses is_writable()
	 */
	function eltd_is_css_folder_writable() {
		$css_dir = get_template_directory().'/css';
...
if(!function_exists('eltd_is_js_folder_writable')) {
	/**
	 ...
	 * @uses is_writable()
	 */
	function eltd_is_js_folder_writable() {
		$js_dir = get_template_directory().'/js';
...
if(!function_exists('eltd_assets_folders_writable')) {
	/**
	 * Function that if css and js folders are writable
	 * @return bool
	 *
	 * @version 0.1
	 * @see eltd_is_css_folder_writable()
	 * @see eltd_is_js_folder_writable()
	 */
	function eltd_assets_folders_writable() {
		return eltd_is_css_folder_writable() && eltd_is_js_folder_writable();
	}
}

if(!function_exists('eltd_writable_assets_folders_notice')) {
	/**
	 ...
	 */
	function eltd_writable_assets_folders_notice() {
		global $pagenow;

		$is_theme_options_page = isset($_GET['page']) && strstr($_GET['page'], 'eltd_theme_menu');

		if($pagenow === 'admin.php' && $is_theme_options_page) {
			if(!eltd_assets_folders_writable()) { ?>
				<div class="error">
	...
	add_action('admin_notices', 'eltd_writable_assets_folders_notice');
}

if(!function_exists('eltd_inline_style')) {
	/**
	 * Function that echoes generated style attribute
	 * @param $value string | array attribute value
	 *
	 * @see eltd_get_inline_style()
	 */
	function eltd_inline_style($value) {
		echo eltd_get_inline_style($value);
	}
}

if(!function_exists('eltd_get_inline_style')) {
	/**
	 ...
	 * @see eltd_get_inline_style()
	 */
	function eltd_get_inline_style($value) {
		return eltd_get_inline_attr($value, 'style', ';');
	}
}

if(!function_exists('eltd_class_attribute')) {
	/**
	 * Function that echoes class attribute
	 * @param $value string value of class attribute
	 *
	 * @see eltd_get_class_attribute()
	 */
	function eltd_class_attribute($value) {
		echo eltd_get_class_attribute($value);
	}
}

if(!function_exists('eltd_get_class_attribute')) {
	/**
	 * Function that returns generated class attribute
	 * @param $value string value of class attribute
	 * @return string generated class attribute
	 *
	 * @see eltd_get_inline_attr()
	 */
	function eltd_get_class_attribute($value) {
		return eltd_get_inline_attr($value, 'class');
	}
}

if(!function_exists('eltd_get_inline_attr')) {
	/**
	 ...
	 */
	function eltd_get_inline_attr($value, $attr, $glue = '') {
		if(!empty($value)) {
...
if(!function_exists('eltd_get_skin_uri')) {
	function eltd_get_skin_uri() {
		global $eltdFramework;
...
if(!function_exists('eltd_post_types_message')) {
    /**
     * Function that prints a mesasge in the admin if user hides TGMPA plugin activation message
     */
    function eltd_post_types_message() {
        if(get_user_meta(get_current_user_id(), 'tgmpa_dismissed_notice', true) && !eltd_post_types_installed()) {
            echo apply_filters('eltd_post_types_message', '<div class="update-nag">'.__('Installation of the "Elated
            CPT" plugin is
            essential for
            proper
    ...
    add_action('admin_notices', 'eltd_post_types_message');
}

if(!function_exists('eltd_post_types_installed')) {
    /**
     * Function that checks if core CPT plugin is installed
     * @return bool whether core plugin is installed
     */
    function eltd_post_types_installed() {
        return defined('ELTD_CORE_VERSION');
    }
}