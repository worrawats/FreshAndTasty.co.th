...
class Elated_Export {
	...
	public function __construct() {
		add_action('admin_menu', array($this, 'eltd_admin_export'));
	}
	 ...
	 */
	public function init_eltd_export() {
	...
	public function export_options($save_file = false) {
		...
		$output = base64_encode(serialize($eltd_options));
	 ...
	 * @return bool | array
	 *
	 * @see Elated_Export::export_sidebars
	 * @see Elated_Export::export_widgets
	 */
	...
	public function export_widgets(){
		...
		$all_eltd_widgets = array();
		
		foreach ($wp_registered_widgets as $eltd_widget_id => $widget_params) {
			$all_eltd_widgets[] = $widget_params['callback'][0]->id_base;
		}

		foreach ($all_eltd_widgets as $eltd_widget_id) {
			$eltd_widget_data = get_option( 'widget_' . $eltd_widget_id );

			if ( !empty($eltd_widget_data) ) {
				$widget_datas[ $eltd_widget_id ] = $eltd_widget_data;
			}

		}

		unset($all_eltd_widgets);
	...
	public function export_sidebars(){
		...
		$eltd_sidebars = $this->exclude_sidebar_keys($eltd_sidebars);

		return $eltd_sidebars;
	}
	 ...
	 */
	public function export_eltd_menus($save_file = false){
		global $wpdb;
	...
	public function export_settings_pages($save_file = false) {
		$eltd_static_page = get_option("page_on_front");
		$eltd_post_page = get_option("page_for_posts");
		$eltd_show_on_front = get_option("show_on_front");

		$eltd_settings_pages = array(
			'show_on_front' => $eltd_show_on_front,
			'page_on_front' => $eltd_static_page,
			'page_for_posts' => $eltd_post_page
		);

		$output = base64_encode(serialize($eltd_settings_pages));
	 ...
	 *
	 * @see Elated_Export::export_settings_pages
	 * @see Elated_Export::export_widgets_sidebars
	 * @see Elated_Export::export_eltd_menus
	 * @see Elated_Export::export_custom_sidebars
	 * @see Elated_Export::export_options
	 */
	public function export_all() {
		...
		$menus_file = $this->export_eltd_menus(true);
	 ...
	 */
	public function eltd_admin_export() {
		if(isset($_REQUEST['export'])) {
			$this->init_eltd_export();
		}

		$this->pagehook = add_menu_page(
			'Elated Theme', esc_html__('Elated Export', 'eltd'),
			'manage_options',
			'eltd_options_export_page',
			array($this, 'eltd_generate_export_page')
		);
	}
	 ...
	 * Method that
	 */
	public function eltd_generate_export_page() {

		?>
		<div id="eltd-metaboxes-general" class="wrap">
					...
					<div id="post-body" class="has-sidebar">
						<div id="post-body-content" class="has-sidebar-content">
							<table class="form-table">
								<tbody>
								<tr><td scope="row" width="150"><h2><?php esc_html_e('Export', 'eltd'); ?></h2></td></tr>
								<tr valign="middle">
									...
									<td>
										...
										<br />
										<form method="post" action="">
											<input type="hidden" name="export_option" value="eltd_options" />
											<input type="submit" value="Export Options" name="export" />
										</form>
										<br />
										<form method="post" action="">
											<input type="hidden" name="export_option" value="eltd_menus" />
...
$my_Elated_Export = new Elated_Export();