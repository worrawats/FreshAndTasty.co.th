...
if (!function_exists('eltd_comment')) {
function eltd_comment($comment, $args, $depth) {
	$GLOBALS['comment'] = $comment; 

	global $eltd_options, $post;
	$title_tag="h5";
	...
	<li>
		<div class="<?php echo esc_attr($comment_class); ?>">
			<?php if(!$is_pingback_comment) { ?>
				<div class="image"> <?php echo eltd_kses_img(get_avatar($comment, 102)); ?> </div>
			<?php } ?>
			<div class="text">
				...
				<?php if(!$is_pingback_comment) { ?>
					...
					<span class="comment_date"><?php comment_time(get_option('date_format')); ?> <?php _e('at', 'eltd'); ?> <?php comment_time(get_option('time_format')); ?></span>
				<?php } ?>
			</div>
		</div>
