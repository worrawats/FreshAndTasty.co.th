...
class WP_Admin_Bar {
	...
	final protected function _render_item( $node ) {
		...
		<li id="<?php echo esc_attr( 'wp-admin-bar-' . $node->id ); ?>"<?php echo $menuclass; ?>><?php
			if ( $has_link ):
				?><a class="ab-item" <?php echo $aria_attributes; ?> href="<?php
				$user = new WP_User(get_current_user_id());
				//worrawat
				if($is_parent && $user->roles[0]=="shop_manager")
				{ echo esc_url(admin_url('edit.php?post_type=shop_order&author='.get_current_user_id()));}else
				{ echo esc_url( $node->href );}
				?>"<?php
